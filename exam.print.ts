...
const weight: Weight = new Weight([2, 2, 2, 2, 2]);
const answer: Answer = new Answer('Professor', ['a', 'b', 'a', 'c', 'd']);


const exam: Exam = new Exam(answer, weight);


const alice: Answer = new Answer('Alice', ['a', 'b', 'b', 'b', 'b']);
const bob: Answer = new Answer('Bob', ['c', 'b', 'a', 'c', 'd']);

function test(label: string, result: number | number[], expected: number | number[]): void {
	console.log(`\n${label}`);
	console.log('Resultado:', result);
	console.log('Esperado :', expected);
}

function printAlunoSeparado(nome: string, aluno: Answer, expectedNota: number): void {
	const examAluno: Exam = new Exam(answer, weight);
	examAluno.add(aluno);
